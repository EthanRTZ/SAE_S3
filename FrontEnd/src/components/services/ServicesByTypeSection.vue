<template>
  <div :class="embedded ? 'services-type-section--embedded' : 'services-type-page'">
    <!-- Loading -->
    <div v-if="loading" :class="['loading-container', { 'loading-container--embedded': embedded }]">
      <p>{{ $t('servicesByType.loading') }}</p>
    </div>

    <!-- Error -->
    <div v-else-if="error" :class="['error-container', { 'error-container--embedded': embedded }]">
    ...
    <div v-else class="services-type-content">
      ...
      <div class="type-header">
        ...
        <div class="type-header-text">
          <h1 v-if="!embedded">{{ localizedLabel }}</h1>
          <h2 v-else class="type-header-title-embedded">{{ localizedLabel }}</h2>
...
const props = defineProps({
  type: {
    type: String,
    required: true,
    validator: (v) => ['reservation', 'commande', 'location'].includes(v)
  },
  embedded: {
    type: Boolean,
    default: false
  },
  /** Sur la page Merch : id du service à ouvrir (query ?service=&type=) */
  openServiceId: {
    type: [String, Number],
    default: null
  },
...
onMounted(async () => {
  const parallel = [loadServices()]
  if (props.type === 'reservation') {
    parallel.push(loadFestivalDates())
  }
  await Promise.all(parallel)

  if (props.embedded) {
    const id = props.openServiceId
    if (id != null && id !== '' && servicesList.value.length) {
      const found = servicesList.value.find(s => String(s.id_service) === String(id))
...
/* Mode intégré (ex. page Merch) */
.services-type-section--embedded {
  min-height: 0;
  padding: 0;
...
.services-type-section--embedded .services-type-content {
  max-width: 1180px;
}

.services-type-section--embedded .type-header {
  margin-bottom: 18px;
  padding: 18px 20px;
}

.services-type-section--embedded .type-header-icon {
  width: 56px;
  height: 56px;
  font-size: 1.75rem;
}

.services-type-section--embedded .type-header-title-embedded {
  font-size: 1.35rem;
  color: #fcdc1e;
  margin: 0 0 6px 0;
}

.services-type-section--embedded .type-description {
  font-size: 0.95rem;
}

.loading-container--embedded,
.error-container--embedded {
  min-height: 100px;
  padding: 20px;
}

.services-type-section--embedded .services-grid {
  grid-template-columns: repeat(auto-fill, minmax(280px, 1fr));
  gap: 14px;
}

.services-type-section--embedded .service-type-card {
  padding: 18px;
}
</style>
